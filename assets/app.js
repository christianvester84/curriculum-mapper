...
(function(){
  // Reference data (ASCII-only short labels)
  var DOMAINS=[
  ...
  var COMP_MAP={
    1:[{code:'1.1',label:'Discipline perspective',subs:[{code:'1.1a',level:'L1'},{code:'1.1d',level:'L2'}]},{code:'1.2',label:'Apply theory & research',subs:[{code:'1.2a',level:'L1'},{code:'1.2e',level:'L2'}]}],
    2:[{code:'2.1',label:'Caring relationship',subs:[{code:'2.1a',level:'L1'},{code:'2.1d',level:'L2'}]},{code:'2.3',label:'Assessment skills',subs:[{code:'2.3a',level:'L1'},{code:'2.3e',level:'L2'}]}],
    3:[{code:'3.1',label:'Manage population health',subs:[{code:'3.1a',level:'L1'},{code:'3.1d',level:'L2'}]},{code:'3.3',label:'Socioeconomic impact',subs:[{code:'3.3a',level:'L1'},{code:'3.3c',level:'L2'}]}],
    4:[{code:'4.2',label:'Integrate best evidence',subs:[{code:'4.2a',level:'L1'},{code:'4.2d',level:'L2'}]}],
    5:[{code:'5.1',label:'Quality improvement',subs:[{code:'5.1a',level:'L1'},{code:'5.1d',level:'L2'}]},{code:'5.2',label:'Patient safety culture',subs:[{code:'5.2b',level:'L1'},{code:'5.2f',level:'L2'}]}],
    6:[{code:'6.1',label:'Communicate for partnership',subs:[{code:'6.1a',level:'L1'},{code:'6.1d',level:'L2'}]}],
    7:[{code:'7.1',label:'Systems thinking',subs:[{code:'7.1a',level:'L1'},{code:'7.1d',level:'L2'}]}],
    8:[{code:'8.1',label:'Use information & tech',subs:[{code:'8.1a',level:'L1'},{code:'8.1e',level:'L2'}]},{code:'8.2',label:'Coordinate care with tech',subs:[{code:'8.2b',level:'L1'},{code:'8.2f',level:'L2'}]}],
    9:[{code:'9.2',label:'Ethical & professional conduct',subs:[{code:'9.2-01',level:'L1'},{code:'9.2-02',level:'L2'}]}],
    10:[{code:'10.1',label:'Self & leadership development',subs:[{code:'10.1a',level:'L1'},{code:'10.1e',level:'L2'}]}]
  };
  ...
  if(!state){ state=JSON.parse(JSON.stringify(DEMO)); persist(); }
  ...
  el('btnExportJSON').onclick=function(){ download('mappings.json', JSON.stringify(state,null,2), 'application/json') };
  ...
  function renderCoverage(){ var rank={I:1,R:2,A:3}; var header='<table class="matrix"><tr><th></th>'+DOMAINS.map(function(d){return '<th>D'+d.id+'</th>'}).join('')+'</tr>'; var rows=state.clos.map(function(clo){ var cells=DOMAINS.map(function(d){ var list=state.mappings.filter(function(m){return m.targetType==='CLO' && m.targetId===clo.id && m.domain===d.id}); var max=''; list.forEach(function(m){ if((rank[m.strength]||0)>(rank[max]||0)) max=m.strength; }); var badge=max?'<span class="badge '+max+'">'+max+'</span>':'-'; return '<td>'+badge+'</td>'; }).join(''); return '<tr><td>'+clo.text.slice(0,28)+'…</td>'+cells+'</tr>'; }).join(''); coverageDiv.innerHTML='<h4>CLO x Domain Coverage</h4>'+header+rows+'</table>'; }
  function renderAssignments(){ var head='<tr><th>Assignment</th><th>Level</th><th>Domain</th><th>Comp</th><th>Sub</th><th>PIs</th></tr>'; var rows=[]; state.assignments.forEach(function(a){ var maps=state.mappings.filter(function(m){return m.targetType==='Assignment' && m.targetId===a.id}); if(!maps.length){ rows.push('<tr><td>'+a.title+'</td><td colspan="5" style="text-align:left;color:#666">— no mapping —</td></tr>'); } else { maps.forEach(function(m,i){ rows.push('<tr><td>'+(i?'':a.title)+'</td><td>'+(m.level||'')+'</td><td>D'+m.domain+'</td><td>'+m.competency+'</td><td>'+m.sub+'</td><td>'+(m.pi||[]).join(', ')+'</td></tr>'); }); }}); assignmentTableDiv.innerHTML='<h4>Assignment Alignment</h4><table class="table">'+rows.join('')+'</table>'; }
  function renderSummaries(){ var cCounts={}, sCounts={}; CONCEPTS.forEach(function(c){cCounts[c]=0}); SPHERES.forEach(function(s){sCounts[s]=0}); state.mappings.forEach(function(m){ (m.concepts||[]).forEach(function(c){cCounts[c]=(cCounts[c]||0)+1}); (m.spheres||[]).forEach(function(s){sCounts[s]=(sCounts[s]||0)+1}); }); function bars(obj){ var max=1; Object.keys(obj).forEach(function(k){ if(obj[k]>max) max=obj[k] }); var out='<ul style="list-style:none;padding-left:0">'; Object.keys(obj).forEach(function(k){ out+='<li style="margin:6px 0">'+k+'<div style="height:8px;background:#e9edf5;border-radius:4px"><div style="height:8px;width:'+((obj[k]/max)*100)+'%;background:'+ '#3561B5' +';border-radius:4px"></div></div></li>'; }); return out+'</ul>'; }
...
'); download('mappings.csv',csv,'text/csv'); }
  ...
  function importJSON(){ var inp=document.createElement('input'); inp.type='file'; inp.accept='.json,application/json'; inp.onchange=function(){ var f=inp.files[0]; if(!f) return; var fr=new FileReader(); fr.onload=function(){ try{ state=JSON.parse(fr.result); persist(); }catch(e){ alert('Invalid JSON'); } }; fr.readAsText(f); }; inp.click(); }
